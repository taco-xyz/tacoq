...
> [!NOTE]
> Note the task field `kind` is set to `"task_wait_n_seconds"`. You can think about it
> the following way:
...
We'll start by setting up the publisher and its configuration:

- It must know how to communicate with the message broker so that it can publish
  tasks to the worker.
- It must know how to communicate with the relay so that it can retrieve the
  results of the task after it has been completed and sent to the relay.
...
Instead, we will simply use the publisher to publish or retrieve a task.
...
Congratulations! You've just published and retrieved a task using TacoQ. You
can keep learning more about TacoQ in the [Technical Reference](/technical-reference)
section.
