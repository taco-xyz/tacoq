...
worker_config = WorkerApplicationConfig(
    name="worker_waiter_1", # The name of the worker for observability purposes.
    kind="worker_waiter_kind", # The kind of worker to route tasks to.
...
<Note>
  Note the task field `kind` is set to `"task_hello_world"`. You can think
