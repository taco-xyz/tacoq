...
```python
from tacoq import TaskInput, TaskOutput, PydanticDecoder
...
@worker_app.task(kind="task_wait_n_seconds", input_decoder=PydanticDecoder(TestInputPydanticModel))
...
<Tip>
  ...
  with the language's most commonly used libraries. These are **well tested and
