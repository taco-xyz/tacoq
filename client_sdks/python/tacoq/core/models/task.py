...
if __name__ == "__main__":
    ...
    print(
        f"Avro: {avro_ser_time * 1000:.1f}ms (Average: {avro_ser_time / N * 1000:.1f}ms, {N / avro_ser_time:.0f} ops/s)"
    )
    print(
        f"JSON: {json_ser_time * 1000:.1f}ms (Average: {json_ser_time / N * 1000:.1f}ms, {N / json_ser_time:.0f} ops/s)"
    )
    print(f"\nDeserialization times ({N} items):")
    print(
        f"Avro: {avro_deser_time * 1000:.1f}ms (Average: {avro_deser_time / N * 1000:.1f}ms, {N / avro_deser_time:.0f} ops/s)"
    )
    print(
        f"JSON: {json_deser_time * 1000:.1f}ms (Average: {json_deser_time / N * 1000:.1f}ms, {N / json_deser_time:.0f} ops/s)"
    )
